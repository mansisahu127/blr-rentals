...
from app.services.rag import search_listings
...
async def chat(request: ChatRequest):
    # Retrieve relevant listings via RAG
    ...
    if listings:
        context_items = []
        for l in listings:
            broker_info = "Direct from Owner (No-Broker)" if l.get('brokerage') else "Listed by Agent (Brokerage applies)"
...
Answer users based ONLY on these listings:
